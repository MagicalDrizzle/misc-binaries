...
int WriteGIFFile( char *name )
{
    ...
    InvalidateCmndLine();
    WriteString(MsgStrs(Out8bitGIF));
    return False;
}
#endif
...
#if defined(THIRTYTWOBIT) && defined (MSWIN) 
int WriteBMP32File(char *name) 
 { 
    ...
    hDC = GetDC(NULL);
    pbih = (PBITMAPINFOHEADER) BitInfo;
    pbih->biSizeImage = ((((pbih->biWidth * pbih->biBitCount) + 31) & ~31) >> 3) * pbih->biHeight;
    BufHandle = (LPBYTE) GlobalAlloc(GMEM_FIXED, pbih->biSizeImage);
    if (!BufHandle) 
         return False; 
    if (!GetDIBits(hDC, PixMap, 0, (WORD) pbih->biHeight, BufHandle, BitInfo, 
        DIB_RGB_COLORS)) 
        return False;
    ...
    fwrite((char*)&bfh,1,(int) (sizeof(BITMAPFILEHEADER)),OutFile);
    fwrite((char*)pbih,1,(int) (sizeof(BITMAPINFOHEADER) 
                  + pbih->biClrUsed * sizeof (RGBQUAD)),OutFile);
    fwrite((char*)BufHandle,1,(int) pbih->biSizeImage,OutFile);

    fclose(OutFile);
    GlobalFree((HGLOBAL)BufHandle);
    ReleaseDC(NULL,hDC);
    return True;
}
...
int WriteBMPFile( char *name )
{
    return WriteBMP32File(name);
}
#else /*THIRTYTWOBIT*/
    UnusedArgument(name);

    InvalidateCmndLine();
    WriteString(MsgStrs(Out8bitBMP));
    return False;
    WriteBMP32File(name);
#endif
...
{
    if( col != VectCol )
    {   fprintf(OutFile,"%g ",(double)RLut[col]/255.0);
        fprintf(OutFile,"%g ",(double)GLut[col]/255.0);
        fprintf(OutFile,"%g ",(double)BLut[col]/255.0);
        fputs("setrgbcolor\n",OutFile);
        VectCol = col;
    }
}
                             ...
                             int index )
{
    ...
    {   fputs("gsave\n",OutFile);
        ...
        fprintf(OutFile,"%g ",(double)RLut[i]/255.0);
        fprintf(OutFile,"%g ",(double)GLut[i]/255.0);
        fprintf(OutFile,"%g ",(double)BLut[i]/255.0);
                           ...
                           int col, int dash )
{
    ...
    {   radius = ((double)dst->radius)*Scale;
    ...
    {   radius = ((double)src->radius)*Scale;
        if( dist <= radius ) return;
                            ...
                            int col, int rad )
{
    ...
    {   midx = 0.5*(src->x + dst->x);
        ...
        {   temp = (Scale*(double)(src->radius))/dist3;
            endx = src->x + temp*dx;
            endy = src->y + temp*dy;
...
static void WriteVectMonitors( void )
{
    ...
    {   s = ptr->src;
        ...
        if( ZValid( (s->z+d->z)/2 ) /*&& ZBack( (s->z+d->z)/2 ) rasmol 2.7.2.1.1 */)
        {   x = (s->x+d->x)/2;
            y = (s->y+d->y)/2;
...
int WriteVectPSFile( char *name )
{
    ...
    if( !data || !type )
    {   InvalidateCmndLine();
        WriteString(MsgStrs(OutNoMemPS));
...
#ifdef INVERT
    fprintf(OutFile,"%d %d translate\n",xpos,ypos);
    fprintf(OutFile,"%g ",(double)xsize/XRange);
    fprintf(OutFile,"%g ",(double)ysize/YRange);
#else
    fprintf(OutFile,"%d %d translate\n",xpos,ypos+ysize);
    fprintf(OutFile,"%g ",(double)xsize/XRange);
    fprintf(OutFile,"%g ",(double)-ysize/YRange);
...
{
#ifdef EIGHTBIT
    if(ABranch) free(ABranch);
    if(DBranch) free(DBranch);
    if(Hash) free(Hash);
    if(Node) free(Node);
#endif
}
